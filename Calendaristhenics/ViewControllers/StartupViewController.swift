...
class StartupViewController: UITableViewController, EKCalendarChooserDelegate {
    ...
    @IBAction func removeAllWorkoutEvents(_ sender: Any) {
        ...
        let endDate = Date()
        let startDate = endDate.addingTimeInterval(-3.9*365*24*60*60) // https://stackoverflow.com/a/20055367/6778597
        let predicate = eventStore.predicateForEvents(withStart: startDate, end: endDate, calendars: [calendar])
        let events = eventStore.events(matching: predicate)
        for event in events {
            if event.notes == "Generated by Calendaristhenics" {
